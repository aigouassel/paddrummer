...
describe('toTimeline', () => {
  it('places 16ths a sixteenth of a beat apart', () => {
    const times = expectedHitTimes(sticking('R L R R'), 120) // 0.5s per beat
    expect(times).toEqual([0, 0.125, 0.25, 0.375])
  })

  it('scales with tempo', () => {
    const slow = expectedHitTimes(sticking('R L'), 60)
    const fast = expectedHitTimes(sticking('R L'), 240)
  ...
  it('does not accumulate error over a bar of triplets', () => {
    ...
    const times = expectedHitTimes(bar, 120)
  ...
  it('counts one expected hit per stroke, ornaments excluded', () => {
    ...
    expect(expectedHitTimes(strokes, 100)).toHaveLength(strokes.length)
    expect(toTimeline(strokes, 100).length).toBeGreaterThan(strokes.length)
  })
})

describe('grace note placement', () => {
  it('puts a flam grace note just before its main note', () => {
    const notes = toTimeline(sticking('l>R'), 120)
  ...
  it('spaces a drag so the last grace note still sits one gap ahead', () => {
    const notes = toTimeline(sticking('llR'), 120)
  ...
  it('keeps flam width constant as tempo changes, because it is a physical gap', () => {
    const width = (bpm: number) => {
      const [grace, main] = toTimeline(sticking('l>R'), bpm)
  ...
  it('never lets a grace note collide with the previous main note', () => {
    ...
    const notes = toTimeline(fullCycle(RUDIMENTS_BY_ID.get('flam-tap')!), 200)
...
describe('input validation', () => {
  it('rejects a non-positive tempo', () => {
    expect(() => toTimeline(sticking('R'), 0)).toThrow(/bpm must be positive/)
  })
})
