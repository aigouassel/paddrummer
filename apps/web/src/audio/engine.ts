import { QUARTER } from "@paddrummer/core/duration";
import type { Fraction } from "@paddrummer/core/fraction";
import {
  type Phrase,
  phraseOfStrokes,
  placedStrokes,
...
import {
  ...
  estimateLatency,
  judge,
} from "@paddrummer/core/scoring";
import type { ExpectedHit, TimedNote } from "@paddrummer/core/timeline";
...
export class PracticeEngine {
  ...
  recordHit(hit: Hit): void {
    ...
    if (this.mode === "calibrating" && this.calibrationComplete())
  ...
  estimate(): LatencyEstimate {
    return estimateLatency(
      this.clickTimes,
      this.rawHits.map((hit) => hit.timeSec),
    );
  }
  ...
  report(): ScoreReport {
    ...
    const settled = this.expected.filter(
      (note) => note.timeSec < settledBefore,
    );
  ...
  get cyclesPlayed(): number {
    ...
    return Math.max(
      0,
      Math.floor((this.currentTimeSec - this.startedAtSec) / cycleSec),
    );
  }
  ...
  private forget(before: number): void {
    while (this.expected.length > 1 && this.expected[0]!.timeSec < before)
      this.expected.shift();
    while (this.rawHits.length > 0 && this.rawHits[0]!.timeSec < before)
      this.rawHits.shift();
  }
